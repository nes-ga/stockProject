...
- 문서를 현재 코드 기준으로 재점검하고 기준일을 2026-08-21로 갱신했습니다.
- 현재 작업 트리에서 과거 손실 종료 이력이 있는 스윙 보유종목도 실제 보유 손익이 수익이면 `SWING_RECOVERED`로 분리해 수익 보호 관찰과 분할매도 계획을 우선하도록 보정했습니다.
- 이 변경은 기존 사용자 작업으로 남아 있으므로 문서 갱신 과정에서 소스 파일을 수정하거나 되돌리지 않았습니다.
- 기본 스윙과 기존 `smallcap` 스윙 엔진은 각각의 스캔 범위, 저장 파일, 히스토리를 유지했습니다.
- 기존 `smallcap` 프로필은 실제 시가총액 구분이 아니라 탐색 임계값 차이이므로 사용자 명칭을 `확장 탐색`으로 변경했습니다.
- 화면의 프로필 탭을 제거하고 두 엔진 결과를 중복 제거한 단일 스윙 목록, 통합 버킷 카운트, 단일 검색 버튼으로 정리했습니다.
- 결과 카드에는 `기본 기준` 또는 `확장 탐색` 배지를 표시해 포착 경로를 추적할 수 있게 했습니다.
- 기존 lead-in 94% 기준은 유지하면서 91~94% 경계 구간 중 후속 상승과 정상 유동성이 재확인된 경우만 `seed_anchor_confirmed` 관찰 후보로 포함했습니다.
- `seed_anchor_confirmed`는 실행 조건과 점수 감점을 우회하지 않으며 `watchItems` 가시성만 보장합니다.
- 한국화장품제조(`003350`)는 기본 기준과 확장 탐색에서 관찰 후보로 확인됐고 실행 후보로는 승격되지 않았습니다.
...
- 중장기 Portfolio 종목에 최근 6개월 일봉 기반 `PortfolioTechnicalSetup`을 추가했습니다.
- SMA20 기울기·이격, 20일 박스 폭, 최근 저점 방어를 조합해 `READY`, `FORMING`, `WAIT`, `UNAVAILABLE` 상태와 기술 무효가를 계산합니다.
- 수익 종목에 3단계 분할매도 목표·수량과 수익보호 가격을 추가했습니다.
- 로컬 OCR 파싱을 보강하고 `npm run verify:portfolio-ocr` 검증 스크립트를 추가했습니다.
- Portfolio Recovery 검증에 기술 박스권, 매도 계획, 계좌·시세 freshness 사례를 확장했습니다.
