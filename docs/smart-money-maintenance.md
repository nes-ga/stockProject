...
- 2026-08-21: 기본 기준과 확장 탐색 엔진·저장 파일·히스토리 범위는 독립적으로 유지하되, 사용자 UI는 프로필 탭을 제거하고 중복 제거된 단일 스윙 목록으로 통합. 검색 버튼은 두 엔진을 함께 실행하고 카드 배지로 포착 기준만 표시. 기존 `smallcap` 식별자는 데이터 호환을 위해 유지하지만 사용자 명칭은 실제 시가총액 분류로 오해하지 않도록 `확장 탐색`을 사용
- 2026-08-21: 기존 lead-in 종가/고가 94% 기준은 유지하고, 91~94% 구간은 후속 1~5거래일 내 추가 상승과 정상 유동성 기준이 재확인된 경우에만 `seed_anchor_confirmed` 관찰 후보로 허용. 이 경로는 낮은 점수나 품질 감점을 우회해 실행 후보로 승격하지 않으며 `watchItems` 가시성만 보장
